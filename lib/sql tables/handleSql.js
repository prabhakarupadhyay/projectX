...
function getShopCountData(connection,tableArr,columns,count,callback){
    ...
    connection.query('SELECT '+columns+'GROUP_CONCAT(Image) Images'+' FROM '+SchemaName.Shop.Schema+' s,'+SchemaName.Shop.ImageSchema+' t WHERE s.Shop_Sub_Id = t.Shop_Sub_Id'+' GROUP BY t.Image',function(err,shopData){
