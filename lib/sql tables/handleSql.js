...
exports.fetchBookDatFromIds = (pool,colNamesArr,idKeyName,idKeyValueArr,schemaName2,callback)=>{
    
    var schemaName1;
    var EmailKey;
    var selectStatement = '';
    
    var splitIdKey = idKeyName.split('_');
    if(splitIdKey[0] == "Shop"){
        schemaName1 = SchemaName.Shop.UserSchema;
        EmailKey = "Shop_User_Email";
    }else{
        schemaName1 = SchemaName.User.Schema;
        EmailKey = "User_Email";
    }
    
    for(var d in colNamesArr){
        selectStatement += schemaName2+'.'+colNamesArr[d]+',';
        if(d == colNamesArr.length-1){
            selectStatement += schemaName1+'.'+EmailKey;
        }
    }
    for(var p in idKeyValueArr){
        
    }
    var whereClause1 = schemaName1+'.'+idKeyName+'='+schemaName2+'.'+idKeyName;
    
    pool.getConnection(function(error,connection){
        connection.query('SELECT '+selectStatement+' FROM '+schemaName2+','+schemaName1+' WHERE '+whereClause1+' AND '+schemaName1+'.'+idKeyName+" IN ('"+idKeyValueArr.join("','")+"') ORDER BY FIELD("+schemaName1+'.'+idKeyName+",'"+idKeyValueArr.join("','")+"')",function(err,IdsData){ 
            if(err){
                connection.release(); 
                callback(undefined);
                console.log(err);
