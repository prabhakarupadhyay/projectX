...
exports.DoneDataBooking = (datastore,curId,curIdKey,kind,entityName,callback)=>{
    
    var keyw = datastore.key([kind, entityName]); 
    datastore.get(keyw, function(err, entity) {
        
        if(entity != undefined){
            var bookingArr = entity[curIdKey+"_"+curId];
            if(bookingArr != undefined && bookingArr[bookingArr.length-1] == true){
                bookingArr[bookingArr.length-1] = false;
                entity[curIdKey+"_"+curId] = bookingArr;
                saveData(datastore,entity,kind,entityName,'update',function(saveResponse){
                    callback("Booking Completed successfully");
                });                 
            }else{
                callback(["Booking already completed"]);
            }
        }else{
            callback(["no slot registered, undefined entity"]);
        }        
