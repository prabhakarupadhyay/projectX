...
function changeEventData(dataChange){
    ...
    function receiveAdminSockets(socketChannelRes,clientObj,pool,datastore,bucket){
        ...
        if(clientObj.Event_Name == "AddCompNumber"){ 
            delete clientObj.Event_Name;
            var numberKey = Object.keys(clientObj);
            companyName = numberKey[0];
            companyNameValue = clientObj[numberKey[0]];
            socketChannelRes.set("Number Added"); 
        }        
        ...
        if(clientObj.Event_Name == "bookingDone"){
            delete clientObj.Event_Name;
            var newBookObj = {};
            if(clientObj.Email_Key_Name == "Shop"){
                newBookObj["Shop_User_Email"] = clientObj.Booking_Email;
                handleSql.fromEmailToId(pool,newBookObj,SchemaName.Shop.UserSchema,function(jsonBook){
                    handleSql.shopsForDisplay(pool,[SchemaName.Shop.Schema],"Shop_Sub_Id","Name",clientObj.Shop_Name,"null",1,function(shopsId){
                        Datastore.DoneDataBooking(datastore,jsonBook.Shop_Id,"Shop_Id",'SHOPS',shopsId[0].Shop_Sub_Id,function(storeRes){ 
                            socketChannelRes.set(storeRes);    
                        });                        
                        
                    });           
                });
            }else if(clientObj.Email_Key_Name == "User"){
                newBookObj["User_Email"] = clientObj.Booking_Email;
                handleSql.fromEmailToId(pool,newBookObj,SchemaName.User.Schema,function(jsonBook){
                    handleSql.shopsForDisplay(pool,[SchemaName.Shop.Schema],"Shop_Sub_Id","Name",clientObj.Shop_Name,"null",1,function(shopsId){ 
                        Datastore.DoneDataBooking(datastore,jsonBook.User_Id,"User_Id",'SHOPS',shopsId[0].Shop_Sub_Id,function(storeRes){ 
                            socketChannelRes.set(storeRes);    
                        });                         
                    });                    
                });
            }else{  
                socketChannelRes.set("incorrect Email Key Name.......type either Shop or User");  
            }
...
function getBookings(datastore,pool,socketChannelRes,shopDatObj,countArr){
    ...
    Datastore.fetchAdminBooking(datastore,shopDatObj[countArr]['Shop_Sub_Id'],function(Shop_Id_Arr,Shop_Booking,User_Id_Arr,User_Booking){
        if(User_Id_Arr != '' && User_Booking != ''){
            handleSql.fetchBookDatFromIds(pool,["Name","Number"],"User_Id",User_Id_Arr,SchemaName.User.PerSchema,function(UserInfo){ 
                ...
                if(Shop_Id_Arr != '' && Shop_Booking != ''){          
                    handleSql.fetchBookDatFromIds(pool,["Name","Number"],"Shop_Id",Shop_Id_Arr,SchemaName.Shop.UserPerSchema,function(shopInfo){
