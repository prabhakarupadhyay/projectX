...
global.priorityEmails = ['1developer.mail@gmail.com'];
...
exports.checkIdSessions= function(req,res,pool,callback){        
    ...
    if(mySession.length != undefined){
        ...
        callback(false,'Welcome Guest:Sign in to book a call');
...
exports.handleOauthSession = function(pool,obj,isOauth,done){
    ...
    isPresentInDb(pool,obj.email,obj.id,isOauth,function(isUntrue){
        ...
        if(isUntrue == true){
         ...
         console.log("tokennnnnnnnnnnnnnnnnnnn sorrrrrrrrrrrrttttttttttttt");
            tokenSorting(obj,relevantDatJson,function(modifiedObj){
                console.log("about to execute sssssssssssssssssqqqqqqqqqqqqqqqqqqqqllllllllllllllllllllllll");
                handleSql.manageQueries(pool,modifiedObj,'userLogin',SchemaName.User.Schema,function(status,dat){
                    if(status){
                        console.log("successssssssssssssfulllllllllllllllllllyy stored");
                        return done(null,obj);
                    }else{
                        //here dat contains error
                         console.log("successssssssssssssfulllllllllllllllllllyy errrrrrrrrrrrrrrrrrrr");
                        return done(dat,null);   
                    } 
            ...
            if(isUntrue == 'falseNormalUser'){  
                ...
                operations.loginUserCredentials(pool,userObj,function(newObj1){
                    return  done(null,newObj1);         
                });
                ...
                console.log('shop owners creds...........'); 
                operations.loginOwnerCredentials(pool,OwnerObj,function(newObj1){
                    return  done(null,newObj1);         
                });
            }
        }       
...
function tokenSorting(oldObj,newObj,callback){
    ...
    if(elementName == 'Shop' || elementName == 'User'){
        ...
        newIdToken = elementName + "_IdToken";
    ...
    if(oldObj.idToken != undefined){
        newObj[newIdToken] = oldObj.idToken;
    }
